...
def callback():
    #----------------------------------------------------------------------
    print ('Отправляется запрос к API: '), e1.get()
    CONSUMER_KEY = ''
    CONSUMER_SECRET = ''
    OAUTH_TOKEN = ''
    OAUTH_TOKEN_SECRET = ''
    ...
    p = json.dumps(tweet)
    res2 = json.loads(p)
    print ('Получено сообщений: '),len(res2['statuses'])
    #----------------------------------------------------------------------
    ...
    while i<len(res2['statuses']):
        ...
        i=i+1
    #-----------------------------------------------------------------------
    print ('Количество ретвитов'), sum(retweet_count),
    print ('Возможный охват'), sum(followers_count)+sum(friends_count)
    ...
    print ('Распределение тональности'), tonal_pos, tonal_value
    w4=Label(window,text=u"РАСПРЕДЕЛЕНИЕ ТОНАЛЬНОСТИ", font = "Times")
    w4.place(relx=0.75, rely=0.01)
    w5=Label(window,text=u"Количество запросов:", font = "Times")
